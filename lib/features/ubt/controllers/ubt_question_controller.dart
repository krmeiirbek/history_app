...
class UBTQuestionController extends GetxController {
  static UBTQuestionController get instance => Get.find();

  final time = 0.obs;
  late Timer timer;
  final questionId = 0.obs;
  ScrollController scrollController = ScrollController();
  late UBTQuizModel quizModel;
  final loading = true.obs;
  final questions = <UBTQuestionModel>[].obs;
  final selectedOptions = <RxList<UBTOptionModel>>[].obs;

  @override
  void onInit() {
    super.onInit();
    quizModel = Get.arguments as UBTQuizModel;
    getQuestionsData();
  }

  void getQuestionsData() async {
    loading.value = true;
    questions.value = await EducationRepository.instance.getUBTQuestions(
      quizModel.subjectId,
      quizModel.quizId,
    );
    startTimer();
    for (final _ in questions) {
      selectedOptions.add(<UBTOptionModel>[].obs);
    }
    loading.value = false;
  }
  ...
  void selectOption(UBTOptionModel option, int index) {
